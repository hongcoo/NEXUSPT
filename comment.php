...
//}
		...
		if($arg["commentpm"] == 'yes' && $CURUSER['id'] != $arr["owner"]&&!$athaveowner&&(!$qoute_cidid||$qoute_cidid&&$qoute_cidid!=$arr["owner"])){
			...
			if($type == "offer"){
			$notifs = sqlesc($lang_comment_target[get_user_lang($arr["owner"])]['msg_offer_receive_comment'] . " [url=offers.php?id=$parent_id&off_details=1] " . $arr['name'] . "[/url].");$subject = sqlesc($lang_comment_target[get_user_lang($arr["owner"])]['msg_offer_receive_comment']."(". $arr['name'] . ")");}
			if($type == "request"){
			$notifs = sqlesc($lang_comment_target[get_user_lang($arr["owner"])]['msg_request_receive_comment'] . " [url=viewrequests.php?id=$parent_id&req_details=1] " . $arr['name'] . "[/url].");$subject = sqlesc($lang_comment_target[get_user_lang($arr["owner"])]['msg_request_receive_comment']."(". $arr['name'] . ")");}
			$identify=sqlesc($type.$parent_id);
